	...
	double Cp, double Cn, int solver_type, double beta)
{
	...
	for(i=0; i<l; i++)
	{
		...
		//add dw dalpha as delta_w delta_alpha, sigma
		while (xi->index != -1)
		{
			double val = xi->value;
			QD[i] += val*val;
	...
	while (!converged && out_iter < max_iter)
	{
		...
		for (int inner = 0;inner<max_inner_iter;inner++)
		{
			...
			for (s=0; s<l; s++)
			{
				...
				while(xi->index!= -1)
				{
					G += dw[xi->index-1]*(xi->value);
					wx += w[xi->index-1]*(xi->value);
					xi++;
			    }

				G = sigma*G*yi+wx*yi-1;

				C = upper_bound [GETI(i)];
				G += dalpha[i]*diag[GETI(i)];

				PG = 0;
				if (dalpha[i] == -alpha[i])        //confused
				{
					if (G < 0)
						PG = G;
				}
				else if (dalpha[i] == C-alpha[i])
				{
					if (G > 0)
						PG = G;
				}
...
//				if(fabs(PG) > 1.0e-12)
//				{
					double dalpha_old = dalpha[i];
					dalpha[i] = min(max(dalpha[i] - G/(sigma*QD[i]), -alpha[i]), C-alpha[i]);    //with c conditon
					d = (dalpha[i] - dalpha_old)*yi;
					xi = prob->x[i];
					while (xi->index != -1)
					{
						dw[xi->index-1] += d*xi->value;
						xi++;
					}
//				}
			}
		...
		for (i=0;i<l;i++)
			alpha_inc[i] = dalpha[i];
		for (i=0;i<w_size;i++)
			allreduce_buffer[i] = dw[i];
/*		if (line_search)
		{
			max_step = INF;
